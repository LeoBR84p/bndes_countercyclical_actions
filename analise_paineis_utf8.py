...
print("=" * 80)
print("ANALISE COMPARATIVA DOS ARQUIVOS PARQUET")
print("=" * 80)
print()
...
print("1. RESUMO GERAL DOS PAINEIS")
print("-" * 80)
...
# ============================================================================
print("\n" + "=" * 80)
print("2. ANALISE DE MUNICIPIOS-ESTADO UNICOS")
print("-" * 80)
...
for name, df in paineis.items():
    if 'municipio' in df.columns and 'estado' in df.columns:
        ...
        print(f"{name:12s}: {len(mun_est):4d} pares municipio-estado unicos")

# Comparar se são iguais
unique_counts = set(mun_est_counts.values())
if len(unique_counts) == 1:
    print(f"\n[OK] TODOS os arquivos tem o MESMO numero de municipios-estado unicos: {unique_counts.pop()}")
else:
    print(f"\n[ALERTA] Os arquivos TEM DIFERENTES numeros de municipios-estado")
...
# ============================================================================
print("\n" + "=" * 80)
print("3. COMPARACAO DE VARIAVEIS POR ARQUIVO")
print("-" * 80)
...
# ============================================================================
print("\n" + "=" * 80)
print("4. TOTAIS DAS VARIAVEIS NUMERICAS (SUM)")
print("-" * 80)
...
for col in sorted(all_numeric_cols):
    ...
    for name in sorted(paineis.keys()):
        if col in numeric_vars[name]:
            ...
            print(f"  {name:12s}: (coluna nao existe)")

# ============================================================================
# 5. ANÁLISE DE VALORES VAZIOS E NÃO VAZIOS
# ============================================================================
print("\n" + "=" * 80)
print("5. REGISTROS VAZIOS E NAO VAZIOS")
print("-" * 80)
...
for name, df in paineis.items():
    ...
    print(f"  Celulas vazias (NaN/null): {null_cells:,} ({null_pct:.2f}%)")
    print(f"  Celulas nao vazias: {non_null_cells:,} ({non_null_pct:.2f}%)")
...
# ============================================================================
print("\n" + "=" * 80)
print("6. ANALISE DE SOBREPOSICAO E DUPLICACAO DE DADOS")
print("-" * 80)
...
print("\n" + "-" * 80)
print("Verificando sobreposicao entre pares de arquivos...")
print("-" * 80)
...
for file1, file2 in combinations(sorted(paineis.keys()), 2):
    ...
    if 'municipio' in common_cols and 'estado' in common_cols:
        ...
        min_comum_cases.append({
            ...
            'interseccao': len(intersection),
            'uniao': len(union),
            'jaccard': jaccard,
            'sobreposicao_%': jaccard * 100
        })

if min_comum_cases:
    print("\nIndice de Jaccard (similaridade: 0=disjunto, 1=identico):")
    for case in min_comum_cases:
        print(f"\n{case['par']}")
        print(f"  Pares em comum: {case['interseccao']}")
        print(f"  Pares unicos (uniao): {case['uniao']}")
        print(f"  Indice Jaccard: {case['jaccard']:.4f} ({case['sobreposicao_%']:.2f}%)")

# ============================================================================
# 7. RECOMENDAÇÃO FINAL
# ============================================================================
print("\n" + "=" * 80)
print("7. ANALISE DE VIABILIDADE DE CONSOLIDACAO")
print("-" * 80)
    ...
    if len(arquivos) == 2:
        ...
        if set(df_principal.columns) & set(df_complementar.columns):
            ...
            if 'municipio' in common_cols and 'estado' in common_cols:
                ...
                print(f"  Sobreposicao (municipio-estado): {overlap}/{max(len(principal_mun_est), len(complementar_mun_est))}")
...
print(f"Total de registros unicos (apos remover exatos): {total_unique_registros:,}")

consolidacao_possivel = total_unique_registros < total_if_simple_merge * 0.9

if consolidacao_possivel:
    print(f"\n[OK] RECOMENDACAO: Ha potencial para consolidacao!")
    print(f"  Reducao possivel: {(1 - total_unique_registros/total_if_simple_merge)*100:.1f}%")
else:
    print(f"\n[ALERTA] Os arquivos tem baixa sobreposicao. Consolidacao recomendada apenas se:")
    print(f"   - As variaveis forem complementares")
    print(f"   - Quiser um unico arquivo de entrada")
